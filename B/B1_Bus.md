...
​	这一节的总线使用的更加规范，其实和`Simple Bus`的区别不是非常的大，只是五个事务更加清晰了，但是还是并行总线，并没有实现串行总线，后续或许可以改成`AXI`，支持突发式传送。

​	`crossXbar`对于多个主设备和从设备实在太友好了，不过如果有很多个模块和设备的话，`Xbar.v`的负担不会过重吗，后续看看是否会有其他方式。

​	好像没什么可说的！就是调bug是在太绕人了，起码比我刚接触总线清晰一些了。
